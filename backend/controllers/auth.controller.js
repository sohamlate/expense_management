...
exports.updateUserDetails = async (req, res) => {
  try {
    ...
    if (!userId || !mobile || !name)
      return res
        .status(400)
        .json({ message: "User ID and mobile are required" });
...
exports.verifyEmail = async (req, res) => {
  try {
    ...
    if (user) {
      return res
        .status(200)
        .json({ message: "Email is already in use", exists: true });
    } else {
      return res
        .status(200)
        .json({ message: "Email is available", exists: false });
    }
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error checking the email", error: error.message });
  }
};
