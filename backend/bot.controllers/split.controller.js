...
class SplitController {
  ...
  async handleGroupSelect(user, text) {
    try {
      ...
      user.markModified("inProgressData");
      console.log("testing length 0", user.inProgressData.participants );
      ...
      this.handler.sendMessage(
        ...
        this.createSharesExampleKeyboard(participants.length,user)
      );
    } catch (error) {
  ...
  async handleManualParticipants(user, text) {
    try {
      ...
      .map(({user}) => ({
        id: user._id,
        display: user.mobile ? `📱 ${user.mobile}` : `📧 ${user.email}`,
      }));
      ...
      user.inProgressData.participants = validParticipants.map(({id}) => id);
      ...
      this.handler.sendMessage(
        ...
        this.createSharesExampleKeyboard(validParticipants.length, user)
      );
    } catch (error) {
      console.log("printingg manually ",error.message);
  ...
  async handleShares(user, text) {
    try {
      let shares = text
        ...
        //   );
        // }
        console.log(shares , "dcsfsdf cds");

      console.log("print testing ", shares.length ,"gap   ", user.inProgressData.participants.length)
  ...
  async finalizeSplit(user, shares) {
    try {
      ...
      console.log("testing length ", user.inProgressData.participants.length ,"gap  ", user);
  ...
  createSharesExampleKeyboard(participantCount, user) {
